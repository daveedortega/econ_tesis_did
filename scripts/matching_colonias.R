...
## Preparar Espacio ----
dev.off()
pacman::p_load(tidyverse,sf,janitor,scales, MatchIt)
rm(list=ls())
options(scipen = 9999)
...
# Write
write_sf(matched_colonias,'output/matched_neighborhoods.shp')

# quick plot
ggplot(matched_colonias[,c('COLONIA','each_thing', 'match', 'geometry')]) +
  geom_sf(aes(fill = each_thing)) 
