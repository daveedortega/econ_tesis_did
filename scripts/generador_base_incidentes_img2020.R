...
inviales_18_22 <- rbind(inviales_15,inviales_18,inviales_21,inviales_22) # 1.9 million obs
rm(inviales_15,inviales_18,inviales_21,inviales_22) # Eliminamos cosas que no necesitamos

# Tabla Incidentes Viales ----
...
l12bfeb <- read_sf("~/Desktop/Mapas/STC_shp/l12b_cerrada_postenero.shp") # Buffer que continua cerrado post enero
...
accidente_auto_cadaver <- test_17_split[[1]] # 1,797 obs
accidente_auto_detencion <- test_17_split[[2]] # 342 obs
accidente_auto_lesionado <- test_17_split[[3]] # 6,054 obs
# atropellados ----
atropellado_cadaver <- test_17_split[[4]] # 2,437 obs
atropellado_detencion <- test_17_split[[5]] # 943 obs
atropellado_lesionado <- test_17_split[[6]] # ------------------ 254,707 Done
atropellado_sismo <- test_17_split[[21]] # 55 obs
# choque con lesionados ----
choque_cl_accidente <- test_17_split[[7]] # -------------------- 482,412 Done
choque_cl_sismo <- test_17_split[[8]] # 575 obs
# choque con prensados ----
choque_cp_accidente <- test_17_split[[9]] # 3,561 obs
choque_cp_sismo <- test_17_split[[10]] # 2 observaciones 
# choques sin lesionados ----
choque_sl_accidente <- test_17_split[[11]] # -------------------- 1,033,593 Done
choque_sl_sismo <- test_17_split[[12]] # 540 obs
# ciclista ----
ciclista_accidente <- test_17_split[[13]] # 10,756 obs
# ferroviario ----
ferroviario_accidente <- test_17_split[[14]] # 232 obs
# Incidentes de transito ----
incidente_transito_micalle <- test_17_split[[15]] # 727 obs
incidente_transito_mitaxi <- test_17_split[[16]] # 71 obs
# Monopatin ----
monopatin_accidente <- test_17_split[[17]] # 125 obs
# Motociclista ----
moto_accidente <- test_17_split[[18]] # --------------------------- 114,258 Done
# Otros ----
otros_accidente <- test_17_split[[19]] # 2,803 obs
# Persona atrapada_desbarrancada ----
atrapada_accidente <- test_17_split[[20]] # 8,670 obs
# vehiculo atrapado ----
vehiculo_atrapado <- test_17_split[[22]] # 1,121 obs
vehiculo_desbarrancado_accidente <- test_17_split[[23]] # 863 obs
vehiculo_atrapado_accidene <- test_17_split[[24]] # 3,258 obs
volcadura_accidente <- test_17_split[[25]] # 44,225
...
# dates_complete <- fechas_secuencia %>% rep(2243) # each county has full dates, 2243 colonias IMS

dates_complete <- fechas_secuencia %>% rep(length(colonias_cdmx$CVE_COL)) # each county has full dates, 1,815 colonias IECM
...
# Sanity Check: Mismo número de incidencias
...
rm(list = setdiff(ls(),c('complete_total', 'complete_moto_accidente', 
                  'complete_atropellado_lesionado', 'complete_choque_cl_accidente', 
                  'complete_choque_sl_accidente')))
# LISTOOOO!!!!
# complete_choque_sl_accidente %>% View()

save(complete_total, 
     complete_moto_accidente, 
     complete_atropellado_lesionado, 
     complete_choque_cl_accidente, 
     complete_choque_sl_accidente,
     file = 'output/bases_completas.RData')
