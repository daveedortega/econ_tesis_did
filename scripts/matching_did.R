...
dev.off()
rm(list = ls())
pacman::p_load(tidyverse, scales, did, beepr, sf)

# Cargar Bases ------------------------------------------------------------

load(file = './output/matched_bases_completas.RData')
...
# save(did_inviales_start, did_inviales_stop, file = './output/matched_did_database.RData')

# Load --------------------------------------------------------------------
rm(list = ls())
load(file = './output/matched_did_database.RData')
...
kableExtra::kable(data.frame(incidente = c('total', 'choque con lesionados', 'choque sin lesionados', 'atropellados', 'accidentes en moto'),
                    ...
                    mutate(att_start = round(att_start, 3), se = round(se, 3)) %>% 
                    mutate(ce = paste0('[', round(att_start - 1.96*se, 2), ' --- ', round(att_start + 1.96*se,2), ']')), 
                  format = 'pipe')

agg_start_complete$DIDparams$data %>% filter(start<85) %>% summarise(mean(total_incidentes))
